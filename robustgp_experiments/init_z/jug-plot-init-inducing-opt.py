...
# )
from jug_init_inducing_opt import (
    init_Z_runs, init_Z_task_results, Ms, dataset_names
)
# Evaluation
init_Z_rmses = {}
init_Z_nlpps = {}
...
# Plotting diff bounds on KL Divergence
for dataset in dataset_names:
    fig, ax = plt.subplots()
    ...
    for method in methods:
        if method in methods_to_ignore:
            continue
        ax.plot(init_Z_Ms[dataset][method], np.array(init_Z_uppers[dataset][method]["Median"]) -
                np.array(init_Z_elbos[dataset][method]["Median"]), '*-', label=name_dict[method],
                color=color_dict[method])
    ax.set_title(plot_title_dict[dataset])
    ax.set_xlabel("M")
    ax.set_ylabel("ELBO - Upper")
    ax.set_ylim(bottom=0)
    plt.tight_layout()
    fig.savefig(f"./figures/opthyp-{dataset}-boundsKL-M={Ms}.pdf")
    # plt.show()
